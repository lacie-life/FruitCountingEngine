...
def resize_bbox(bbox, in_size, out_size):
    ...
    np.array(bbox)[:, 0] = y_scale * np.array(bbox)[:, 0]
    np.array(bbox)[:, 2] = y_scale * np.array(bbox)[:, 2]
    np.array(bbox)[:, 1] = x_scale * np.array(bbox)[:, 1]
    np.array(bbox)[:, 3] = x_scale * np.array(bbox)[:, 3]
    return bbox.copy()
...
def flip_bbox(bbox, size, y_flip=False, x_flip=False):
    ...
    bbox = bbox.copy()
    if y_flip:
        y_max = H - np.array(bbox)[:, 0]
        y_min = H - np.array(bbox)[:, 2]
        np.array(bbox)[:, 0] = y_min
        np.array(bbox)[:, 2] = y_max
    if x_flip:
        x_max = W - np.array(bbox)[:, 1]
        x_min = W - np.array(bbox)[:, 3]
        np.array(bbox)[:, 1] = x_min
        np.array(bbox)[:, 3] = x_max
    return bbox.copy()
