...
#ifndef MO_TRACKER_PIPELINE_H
#define MO_TRACKER_PIPELINE_H
        ...
        while (true) {
            ...
            if(zed.grab() == sl::ERROR_CODE::SUCCESS)
            {
                ...
                if(enableCount)
                {
                    ...
                    // Update Counter
                    CounterUpdater(left_cv_rgb, countObjects_LefttoRight, countObjects_RighttoLeft);
                    tCounting += cv::getTickCount() - tStartCounting;

                    if(drawCount){
                        DrawCounter(left_cv_rgb, fontScale, countObjects_LefttoRight, countObjects_RighttoLeft);
                    }

                }

                if(drawOther){
                    DrawData(left_cv_rgb, frameCount, fontScale);
                }
